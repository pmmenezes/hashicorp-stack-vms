...
# tls
# To secure RPC communications for Consul clients consul provides two different approaches,
# the auto encryption mechanism and the manual configuration.
# Both confguration to Auto encryption e Manual configuration on server

tls {
   defaults {
      ca_file = "/etc/consul.d/certs/consul-agent-ca.pem" #Specifies the path to the CA public certificate file.
      cert_file = "/etc/consul.d/certs/dc1-server-consul-0.pem" #Specifies the path to the agents public certificate file.
      key_file = "/etc/consul.d/certs/dc1-server-consul-0-key.pem" #Specifies the path to the agents certificate private key file.


      verify_incoming = true # If set to true, Consul requires all incoming connections to use TLS.
      verify_outgoing = true # If set to true, Consul requires that all outgoing connections from this agent use TLS.
   }
   internal_rpc {
...
#domain
# If using a custom domain for DNS queries, specify the domain field as well. 
# This should match the -domain flags that were used when generating TLS credentials in Generate TLS certificates for RPC encryption
#domain = "example.com"
...
# consul  service mesh
# https://developer.hashicorp.com/consul/tutorials/production-deploy/deployment-guide#consul-service-mesh
# connect.enabled 
...
# addresses.grpc 
# - The address that Consul will bind gRPC API to. 
#   Defaults to client_addr but it might be sensitive to have it on localhost/127.0.0.1 for security reasons.
#   ports.grpc 
# - The gRPC API port. We recommend using 8502 for grpc by convention as some tooling will work automatically with this. 
